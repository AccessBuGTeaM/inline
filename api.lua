...
local json = (loadfile "/home/anti/inline/JSON.lua")()
local token = '419654158:AAF1c1PfdAIiTB4bzgZEHyc759oKJEaOrks' --token
local url = 'https://api.telegram.org/bot' .. token
local offset = 0
local redis = require('redis')
local redis = redis.connect('127.0.0.1', 6379)
local SUDO = 366441416
function is_mod(chat,user)
sudo = {366441416,325007537}
  local var = false
        ...
        for i=1, #updates.result do
          ...
          if msg.inline_query then
            local q = msg.inline_query
						if q.from.id == 362530388 or q.from.id == 366441416 then
 ...
 if redis:get(hash) then
    ...
    return false
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'[🔖Send Your Msg🔖](https://telegram.me/Access_Denied_Bot)',keyboard)
            end
							------------------------------------------------------------------------
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'[✔️Send Your Problem✔️](https://telegram.me/Access_Denied_Bot)',keyboard)
            end
							------------------------------------------------------------------------
		...
		local keyboard = {}
              ...
              edit(q.inline_message_id,'[❗️Send Your Offer❗️](https://telegram.me/Access_denied_bot)',keyboard)
            end
							------------------------------------------------------------------------
